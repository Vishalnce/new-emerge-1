...
const Path = () => {
  ...
  // Function to handle user clicking on a topic
  const handleTopicClick = (index: number) => {
    clearInterval(intervalRef.current!); // Stop the animation loop
    setIsPaused(true); // Mark as paused
    setCurrentIndex(index); // Show selected topic

    // Restart the animation after 4 seconds
        ...
        <div className="m-3 flex flex-row justify-between max-md:flex-col items-center">
          {info.map((item, index) => (
            <div
              ...
              className={`cursor-pointer flex flex-row w-[20%] max-sm:w-[70%] max-sm:m-4 justify-center border-animated p-2 ${
                index === currentIndex
                  ? "border-b-[3px] border-[#C8C7CC]"
                  : "border-b-[3px] border-[#6466366]"
              }`}
            >
              <img src={`path${index + 1}.svg`} alt={`path${index + 1}`} />
              <p className="m-2 my-auto font-semibold text-sm">{item.topic}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-row max-md:flex-col-reverse justify-between items-center h-auto mb-7 max-sm:mb-12">
          ...
          <div className="w-[20%] h-auto max-sm:w-[60%] bg-[white] shadow-lg pb-3">
            ...
            <div className="flex flex-row items-center justify-center">
              <Link href="/soft-skills" className="w-full flex justify-center">
          ...
          <div className="w-[70%] h-[60vh] max-sm:h-[30vh] bg-[url('/vector1.svg')] bg-contain bg-no-repeat bg-center relative max-sm:hidden">
            {[
              info[currentIndex].sk1,
              info[currentIndex].sk2,
              info[currentIndex].sk3,
            ].map((skill, i) => (
              <div
                key={i}
